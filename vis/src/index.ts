...
// SCENE
let width = window.innerWidth, height = window.innerHeight, delta: number;
...
// NAVIGATION
const wKey = new KHold( KEYCODE.W, 16.666 ), aKey = new KHold( KEYCODE.A, 16.666 ), 
      sKey = new KHold( KEYCODE.S, 16.666 ), dKey = new KHold( KEYCODE.D, 16.666 );
...
downKey.addEventListener ('holding', function( event:any ) { 
    cameraControls.rotate(0,0.05 * THREE.MathUtils.DEG2RAD * event.deltaTime, true ) } );
// SETTINGS
const rKey = new KHold( KEYCODE.R, 16.666 ), gKey = new KHold( KEYCODE.G, 16.666 ), 
      hKey = new KHold( KEYCODE.H, 16.666 ), oneKey = new KHold( KEYCODE.ONE, 16.666 );
...
gKey.addEventListener( 'holdEnd', function() { 
    grid.visible = !grid.visible; factoryLine.visible = !factoryLine.visible; }); 
hKey.addEventListener('holdEnd', function() { 
...
async function getControllers() {
    ...
    return data.reduce((acc: { [key: string]: Controller }, item: any) => {
        acc[item.name] = {
            name: item.name, comm: item.comm, pos: item.pos, orient: item.orient, 
            ip: item.ip, signal: item.signal,
        };
        return acc;
    }, {});
}
    ...
    gltfLoader.load(
        ...
        (gltf: any) => {
            ...
            cameraControls.setLookAt(
                camPos.x,camPos.y,camPos.z,center.x,center.y,center.z);
            ...
            const gridMaterial = new THREE.MeshBasicMaterial(
                {color:cDark,side:THREE.DoubleSide});
...
async function loadControllers() {
    ...
    const mesh = new THREE.InstancedMesh(sphereGeometry, material, 
